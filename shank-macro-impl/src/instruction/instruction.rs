...
use super::{
    account_attrs::{AccountsSource, InstructionAccount}, IdlInstruction, InstructionStrategies,
    InstructionStrategy,
};
...
impl TryFrom<&ParsedEnumVariant> for InstructionVariant {
    ...
    fn try_from(variant: &ParsedEnumVariant) -> ParseResult<Self> {
        ...
        let (accounts_source, accounts, strategies) = match IdlInstruction::try_from(attrs) {
            Ok(idl_ix) => {
                ...
                let accounts = match &accounts_source {
                    ...
                    AccountsSource::Struct(path) => {
                        ...
                        vec![InstructionAccount {
                            ident: ident.clone(),
                            index: Some(0),
                            name: format!("__accounts_struct_{}", path.get_ident().unwrap_or(&ident)),
                            writable: false,
                            signer: false,
                            optional_signer: false,
                            desc: Some(format!("Accounts defined by struct: {}", path.get_ident().unwrap_or(&ident))),
                            optional: false,
                        }]
                    }
                };
