...
fn test_different_lifetime_names() {
    ...
    pub struct CustomLifetimeAccounts<'a> {
        #[account(signer)]
        pub _authority: &'a AccountInfo<'a>,

        #[account(mut)]
        pub _data: &'a AccountInfo<'a>,
    }

    let metadata = CustomLifetimeAccounts::__shank_accounts();
    assert_eq!(metadata.len(), 2);
    assert_eq!(metadata[0].1, "_authority");
    assert_eq!(metadata[1].1, "_data");
}
...
fn test_no_constraints_with_references() {
    ...
    pub struct SimpleAccounts<'info> {
        pub _read_only1: &'info AccountInfo<'info>,
        pub _read_only2: &'info AccountInfo<'info>,
    }
