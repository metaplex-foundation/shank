...
fn test_conflicting_constraints_compile_time_error() {
    ...
    pub struct ValidAccounts<'info> {
        ...
        pub optional_signer_account: Option<&'info AccountInfo<'info>>,

        #[account(mut, signer)] // This combination is valid
        pub mut_signer: &'info AccountInfo<'info>,
    }
...
fn test_complex_constraint_combinations() {
    ...
    pub struct ComplexAccounts<'info> {
        ...
        #[account(mut)]
        pub mut_only: &'info AccountInfo<'info>,

        #[account(signer)]
        pub signer_only: &'info AccountInfo<'info>,

        #[account(optional)]
        pub optional_only: Option<&'info AccountInfo<'info>>,

        #[account(optional_signer)]
        pub optional_signer_only: Option<&'info AccountInfo<'info>>,

        #[account(mut, signer)]
        pub mut_signer: &'info AccountInfo<'info>,

        #[account(mut, optional)]
        pub mut_optional: Option<&'info AccountInfo<'info>>,

        #[account(mut, signer, desc = "Complex account")]
        pub mut_signer_desc: &'info AccountInfo<'info>,
    }
...
fn test_different_lifetime_names() {
    ...
    pub struct CustomLifetime<'my_lifetime> {
        pub account: &'my_lifetime AccountInfo<'my_lifetime>,
    }
...
fn test_underscore_and_special_field_names() {
    ...
    pub struct SpecialNames<'info> {
        pub _underscore: &'info AccountInfo<'info>,
        pub camelCase: &'info AccountInfo<'info>,
        pub snake_case: &'info AccountInfo<'info>,
        pub UPPER_CASE: &'info AccountInfo<'info>,
        pub account123: &'info AccountInfo<'info>,
    }
...
fn test_long_descriptions() {
    ...
    pub struct LongDescriptions<'info> {
        #[account(
            desc = "This is a very long description that goes on and on and explains exactly what this account is used for in great detail with multiple sentences and lots of information."
        )]
        pub detailed_account: &'info AccountInfo<'info>,

        #[account(desc = "")]
        pub empty_desc: &'info AccountInfo<'info>,

        #[account(desc = "Short")]
        pub short_desc: &'info AccountInfo<'info>,
    }
