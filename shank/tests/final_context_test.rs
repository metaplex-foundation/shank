...
fn test_minimal_accounts() {
    ...
    // Provide all accounts including program ID as placeholder for optional
    let program_id_account = AccountInfo {
        key: &ID,
        data: &[],
        owner: &[0; 32],
    };

    let accounts = [payer, data, program_id_account];
    ...
    assert_eq!(ctx.accounts.payer.key, &payer_key);
    assert_eq!(ctx.accounts.data.key, &data_key);
    assert!(ctx.accounts.optional_account.is_none()); // Should be None because key == program_id
}
